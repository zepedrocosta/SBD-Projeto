...
def load_data():
    ...
    # Directory 2: csv_default_cnf files
    dir2_summary = pd.read_csv(f"{base_path}/csv_default_cnf/hammerdb_summary.csv")
    ...
    for db in databases:
        try:
            ...
            dir2_counts[db] = pd.read_csv(
                f"{base_path}/csv_default_cnf/hdbtcount_{db}_12c.csv"
            )
...
def create_summary_chart(dir1_summary, dir2_summary, output_path):
    ...
    summary_path = os.path.join(output_path, "hammerdb_summary_comparison_dcnf.png")
    ...
    for db in dir1_counts.keys():
        if db in dir2_counts and len(dir1_counts[db]) > 0 and len(dir2_counts[db]) > 0:
            ...
            count_path = os.path.join(
                output_path, f"dcnf_database_count_comparison_{db}.png"
            )
